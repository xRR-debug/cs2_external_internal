...
namespace OSImGui
{
    ...
    {
        if (!CallBack)
            throw OSException(X("CallBack is empty"));
        if (WindowName.empty())
            Window.Name = X("Angeldancy");

        Window.Name = WindowName;
        Window.wName = StringToWstring(Window.Name);
        Window.ClassName = X("AngeldancyWindowClass");
        ...
        if (!CreateMyWindow())
            throw OSException(X("CreateMyWindow() call failed"));
    ...
    void  OSImGui_External::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        ...
        if (DestWindowName.empty() && DestWindowClassName.empty())
            throw OSException(X("DestWindowName and DestWindowClassName are empty"));
        ...
        if (DestWindow.hWnd == NULL)
            throw OSException(X("DestWindow isn't exist"));
    ...
    void OSImGui_External::setupwindow()  //hooking to geforce overlay you can change to create window, or anything (overwatch's anti cheat isnt kernel level)
    {
        Window.hWnd = FindWindowA(("asd"), ("OpenTabletDriver v0.6.4.0"));
        //atieclxx.exe
        if (!Window.hWnd)
        {
            system("cls");
        ...
        SetWindowLongA(Window.hWnd, GWL_EXSTYLE, WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW | WS_EX_LAYERED);
        SetWindowLongA(
            Window.hWnd,
            -20,
            static_cast<LONG_PTR>(
                static_cast<int>(GetWindowLongA(Window.hWnd, -20)) | 0x20
                )
        );
